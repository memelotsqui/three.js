...
class GLTFParser {
    ...
    parse(onLoad, onError) {
        ...
        Promise.all(this._invokeAll(function(ext) {
            ...
            return Promise.all([
                ...
                parser.getDependencies('animationController'),
            ...
            const result = {
                ...
                animationControllers: dependencies[7],
     ...
     */
    getDependency(type, index, nodeindex) {
        const nodeCache = nodeindex == null ? "" : ':' + nodeindex;

        const cacheKey = type + ':' + index + nodeCache;
        ...
        if (!dependency) {
            ...
            switch (type) {
                ...
                case 'animationController':
                    dependency = this.loadAnimationController(index, nodeindex);
                    break;
    ...
    getDependencies(type) {
        ...
        if (!dependencies) {
            ...
            if (type !== "animationClip" && type !== "animationController" && type !== "cubeTexture") {
                const parser = this;
    ...
    assignTexture(materialParams, mapName, mapDef) {
        ...
        return this.getDependency('texture', mapDef.index).then(function(texture) {
            ...
            if (mapDef.texCoord !== undefined && mapDef.texCoord != 0 && !(mapName === 'lightMap' || mapName === 'aoMap' && mapDef.texCoord == 1)) {
    ...
    //custom
    assignFinalSubmeshMaterials(mesh) {
        ...
        if (mesh.isPoints) {
            ...
            for (let i = 0, il = materials.length; i < il; i++) {
                ...
                if (!pointsMaterial) {
                    ...
                    mesh.material[i] = pointsMaterial;
                }
        ...
        if (useDerivativeTangents || useVertexColors || useFlatShading) {
            ...
            for (let i = 0, il = materials.length; i < il; i++) {
                ...
                if (!cachedMaterial) {
                    ...
                    mesh.material[i] = cachedMaterial;
        ...
        if (usesAO === true && geometry.attributes.uv2 === undefined && geometry.attributes.uv !== undefined) {
    ...
    loadMesh(meshIndex) {
        ...
        const extras = meshDef.extras === undefined ? {} : meshDef.extras;
        const submeshes = extras.submeshes;
        ...
        //custom
        if (extras.submeshes !== undefined) {

            for (let i = 0, il = submeshes.length; i < il; i++) {

                const material = submeshes[i].material === undefined ?
                    createDefaultMaterial(this.cache) :
                    this.getDependency('material', submeshes[i].material);
        ...
        //pendingPrimitives.push( parser.loadGeometries( primitives ) );
        const pendingPrimitives = parser.loadGeometries(primitives);

        // 		return Promise.all( [

        // 	Promise.all( pendingPrimitives ),
        // 	Promise.all( pendingMaterials )

        // ] ).then( function ( dependencies ) {

        // 	const prim = dependencies[ 0 ];
        // 	const mats = dependencies[ 1 ];
            ...
            for (let i = 0, il = geometries.length; i < il; i++) {
                ...
                if (primitive.mode === WEBGL_CONSTANTS.TRIANGLES ||
                    ...
                    if (submeshes !== undefined) {
                        let start = 0;
                        for (let i = 0, il = submeshes.length; i < il; i++) {
                            mesh.geometry.addGroup(start, submeshes[i].count, i);
                            start += submeshes[i].count;
                        }
                    }
                ...
                if (submeshes === undefined) {
                    ...
                    parser.assignFinalSubmeshMaterials(mesh);
                }
